...
class Bottleneck(BaseModule):
    """Bottleneck block for TwinResNet.
...
class TwinResNet(BaseModule):
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            # rgb encoder
            res_layer_x = self.make_res_layer(
                block=self.block,
            ...
            # ano encoder
            res_layer_y = self.make_res_layer(
                block=self.block,
                inplanes=self.inplanes,
                planes=planes,
                num_blocks=num_blocks,
            ...
            layer_x_name = f'layer_x{i + 1}'
            layer_y_name = f'layer_y{i + 1}'
            self.add_module(layer_x_name, res_layer_x)
            self.add_module(layer_y_name, res_layer_y)
            self.res_layers_x.append(layer_x_name)
            self.res_layers_y.append(layer_y_name)
    ...
    def make_stage_plugins(self, plugins, stage_idx):
        ...
        for plugin in plugins:
            ...
            if stages is None or stages[stage_idx]:
                stage_plugins_x.append(plugin)
                stage_plugins_y.append(plugin)

        return stage_plugins_x, stage_plugins_y
    ...
    @property
    def norm1_x(self):
        """nn.Module: the normalization layer named "norm1_x" """
        return getattr(self, self.norm1_x_name)

    @property
    def norm1_y(self):
        """nn.Module: the normalization layer named "norm1_y" """
        return getattr(self, self.norm1_y_name)

    def _make_stem_layer(self, in_channels, stem_channels):
        """Make stem layer for ResNet."""
        if self.deep_stem:
            # rgb encoder
            self.stem_x = nn.Sequential(
                build_conv_layer(
                    self.conv_cfg,
                    in_channels,
                    stem_channels // 2,
            ...
            self.conv1_y = build_conv_layer(
                ...
                padding=3,
                bias=False)
            self.norm1_x_name, norm1_x = build_norm_layer(
                self.norm_cfg, stem_channels, postfix='1_x')
            self.norm1_y_name, norm1_y = build_norm_layer(
                self.norm_cfg, stem_channels, postfix='1_y')
            self.add_module(self.norm1_x_name, norm1_x)
            self.add_module(self.norm1_y_name, norm1_y)
            self.relu_x = nn.ReLU(inplace=True)
            self.relu_y = nn.ReLU(inplace=True)
        self.maxpool_x = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.maxpool_y = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def _freeze_stages(self):
        ...
        if self.frozen_stages >= 0:
            if self.deep_stem:
                self.stem_x.eval()
                self.stem_y.eval()
                for param in self.stem_x.parameters():
                    param.requires_grad = False
                for param in self.stem_y.parameters():
                    param.requires_grad = False
            else:
                self.norm1_x.eval()
                self.norm1_y.eval()
                for m in [self.conv1_x, self.norm1_x]:
                    for param in m.parameters():
                        param.requires_grad = False
                for m in [self.conv1_y, self.norm1_y]:
                    for param in m.parameters():
                        param.requires_grad = False

        for i in range(1, self.frozen_stages + 1):
            m_x = getattr(self, f'layer_x{i}')
            m_x.eval()
            m_y = getattr(self, f'layer_y{i}')
            m_y.eval()
    ...
    def forward(self, x):
        ...
        outs_y = []
        # rgb encoder
        for i, layer_name in enumerate(self.res_layers_x):
            res_layer_x = getattr(self, layer_name)
            x = res_layer_x(x)
            if i in self.out_indices:
                outs_x.append(x)
        # ano encoder
        for i, layer_name in enumerate(self.res_layers_y):
            res_layer_y = getattr(self, layer_name)
            y = res_layer_y(y)
            if i in self.out_indices:
                outs_y.append(y)
